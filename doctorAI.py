...
def train_doctorAI(
	...
	if predictTime and embFineTune:
		...
		help= [p for p in tparams.values()]
		grads = T.grad(cost, wrt=help)
	...
	for epoch in range(max_epochs):
		...
		for index in random.sample(range(n_batches), n_batches):
			...
			f_update()
			if (iteration % 10 == 0) and verbose: print ('epoch: ',epoch, 'iteration: ',iteration/n_batches, 'cost: ', cost)
			iteration += 1

		print ('epoch: ', epoch, 'mean_cost: ',np.mean(costVector))
		use_noise.set_value(0.)
		validAuc = calculate_auc(test_model, validSet, options)
		print ('Validation cross entropy: ', validAuc,'at epoch: ', epoch)
		if validAuc < bestValidCrossEntropy: 
			...
			print ('Test cross entropy: ',testCrossEntropy, 'at epoch: ', epoch)
			tempParams = unzip(tparams)
			np.savez_compressed(outFile + '.' + str(epoch), **tempParams)
	print ('The best valid cross entropy: ',bestValidCrossEntropy, 'at epoch: ', bestValidEpoch)
	print ('The test cross entropy: ', testCrossEntropy)
...
if __name__ == '__main__':
	...
	train_doctorAI(
		seqFile='output.seqs', #args.seq_file, 
		inputDimSize=6, #args.n_input_codes, 
		labelFile='output.seqs', #args.label_file, 
		numClass=6, #args.n_output_codes, 
		outFile='model',#args.out_file, 
